...
declare function browse:get-all($node as node(), $model as map(*), $collection as xs:string*, $element as xs:string?, $facets as xs:string?){
    ...
    (: Order every entity browse page A -> Z by the record's visible display name.
       data:get-records() keys its own order-by on ft:field($hit,"title"), which is
       scoped to tei:body and therefore empty for header-titled records (persons,
