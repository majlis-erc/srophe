                            ...
                            <xsl:when test="descendant::t:ptr[@target and starts-with(@target, concat($base-uri,'/bibl/'))]">
                                ...
                                <xsl:choose>
                                    <xsl:when test="doc-available($biblfilepath)">
                                        ...
                                        <xsl:choose>
                                            <xsl:when test="$rec//t:bibl[@type='formatted'][@subtype='bibliogrpahy']">
                                              <xsl:apply-templates select="$rec//t:bibl[@type='formatted'][@subtype='bibliogrpahy']"/>
                                            </xsl:when>
                                            <xsl:otherwise>
                   ...
                   <xsl:variable name="biblfilepath">
                    ...
                    <xsl:choose>
                        <xsl:when test="doc-available($biblfilepath)">
                            <xsl:variable name="rec" select="document($biblfilepath)"/>
                            <xsl:choose>
                                <!-- Add for https://github.com/majlis-erc/srophe/issues/130 -->
                                <xsl:when test="$rec/descendant::t:bibl[@subtype='bibliography']">
                                    <xsl:for-each select="$rec/descendant::t:bibl[@subtype='bibliography']">
                                        <xsl:apply-templates/>    
                                    </xsl:for-each>
